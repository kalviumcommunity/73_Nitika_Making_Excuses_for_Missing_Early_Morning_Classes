...
//   try {
//       const users = await User.find(); // ✅ Correctly fetch all users
//       res.json(users);
//   } catch (err) {
//       res.status(400).json({ error: err.message });
//   }
...
//     try{
//         const updatedUser = await User.findByIdAndUpdate(req.params.id, req.body, {new: true}); //find by id and update not delete
...
//         const updatedExcuse = await Excuse.findByIdAndUpdate(req.params.id, { likes }, { new: true });
